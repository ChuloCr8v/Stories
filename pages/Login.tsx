...
const Login :FC = (props) => {
  ...
  return(
    ...
    {showToast ? <Toast msg={user ? "Login Successful!" : "error, Login failed. Please try one more time"} closeIcon=<FaTimes className={styles.close_icon} onClick={() => setShowToast(false)} /> icon={user ? <FaCheck className={styles.icon} /> : <FaTimes className={styles.icon} />}  /> : ''} 
      ...
      <form action={props.action} className={styles.form} >
       ...
       <Button text={"Login"} onClick={(e) => login(e, email, password)} />
       <p className={styles.login_url} >Don't have an account? <span><Link href="/Signup"><a className={styles.url}>Signup here </a></Link></span></p>
      </form>
    </div>
    </div>
    )
